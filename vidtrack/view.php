...
<script type="text/javascript">
 ...
 function onPlayerStateChange(event) {
	   ...
	   var state=event.data;
	  	var curtime=player.getCurrentTime().toFixed(0);;
		//number_format($curtime,2); 
		//$curtime = round($var, 2)
	//alert (curtime);
	   //document.getElementById("demo").innerHTML="State changed "+event.data+player.getVideoUrl()+player.getCurrentTime()+" "+course;
		
	   xhttp = new XMLHttpRequest();
	   xhttp.open("GET", "add_data.php?state="+state+"&course="+course+"&user="+user+"&video="+video+"&temp="+curtime, true);
	   xhttp.send();
  } 
//GET VIDEO DURATION
 ...
 // $userr=$USER->id;
 // echo $userr;
 $result = $DB->get_records("youtube" , array()); // get all records in mdl_user table
// $records = $DB->get_records_sql('SELECT time_state,state From {youtube} where user='.$userr.' ORDER BY id DESC LIMIT 1 ' );
// foreach($records as $record){

     
	 // $timestat=$record->time_state;
	 // echo $timestat;
     // echo $record->state;
